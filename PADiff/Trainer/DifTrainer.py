...
class DifTrainer_bc:
    def __init__(self, frame_work, device, max_ep_len, sample_num, num_epochs, action_dim, logger):
    ...
    def train(self, train_loader, epoch):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch+1}/{self.num_epochs}") as pbar:
            for batch_idx, episodes_data in enumerate(pbar):
                action_loss = self.train_step(episodes_data)
        ...
        self.logger.info(f"Epoch [{epoch+1}/{self.num_epochs}] completed. Average Action Loss: {avg_epoch_action_loss :.4f}")
        self.logger.info("===================================================================================")
        return {"action_loss": avg_epoch_action_loss.detach().cpu().item()}

    def train_step(self, episodes_data):
        self.frame_work.model.train()
        sample_list = random.sample(range(0, self.max_ep_len), self.sample_num)
        action_loss = 0.0
        for ts in sample_list:

            s = episodes_data["state"][:, ts, :, :].to(self.device).permute(1, 0, 2)  # shape [batch, num, dim]
            o = episodes_data["obs"][:, ts, :].to(self.device)          # shape [batch, dim]
            rtg = episodes_data["rtg"][:, ts].to(self.device)       # shape [batch, 1]
...
class DifTrainer:
    ...
    def mid_loss(self, g, rtg, mid, s):
        rtg_hat = self.frame_work.RtgNet(state=s, h=mid)
        g_hat = self.frame_work.ReconGoal(state=s, r_hat=rtg_hat)
